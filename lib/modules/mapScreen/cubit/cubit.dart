...
class MapScreenCubit extends Cubit<MapScreenStates> {
  ...
  void getPolyPoints(item)async{
    PolylinePoints polyPoints = PolylinePoints();
    PolylineResult result = await polyPoints.getRouteBetweenCoordinates(
        google_Api_key,
      PointLatLng(position!.latitude, position!.longitude),
      PointLatLng(
        double.parse(data[item].lap![item].tappaLat.toString()),
        double.parse(data[item].lap![item].tappaLng.toString(),
      ),
    )
    );
...
///Custom Marker
  ...
  ///GetTapMarkers
  Future<Set<Marker>> getTapMarkers(context) async {
    print(data.length);
    if (data.isNotEmpty) {
      for (int item = 0; item < data.length; item++) {
        markers.add(Marker(
          position: LatLng(double.parse(data[item].lap![item].tappaLat.toString()),
              double.parse(data[item].lap![item].tappaLng.toString())),
          markerId: MarkerId('2022'),
          onTap: () {
            showFlexibleBottomSheet(
              isExpand: true,
              bottomSheetColor: Colors.transparent,
          ...
          infoWindow: InfoWindow(
            title: data[item].lap![item].nameTappa.toString(),
          ),
          icon:await BitmapDescriptor.fromAssetImage(
              const ImageConfiguration(size: Size(50, 50)), 'assets/images/marker.png'
          ),
