...
class TimeScreenCubit extends Cubit<TimeScreenStates> {
  ...
  /////////////////////////////////
  static const countDowenDuration = Duration(minutes: 600);
...
//bool addExtraTime = false;
  ...
  Future<void> saveTimeAndRouteIDAndTapId({
    ...
    DioHelper.postData(methodUrl: POST_TIME, data: {
      "route_id": route_id,
      "Tap_id": Tap_id,
      "Total_Time_Tap": Time,
    }).then((value) {
...
//End POST TIME AND ROUTE_ID AND TAP_ID FUNCTION
  ...
  Future<void> deleteDataOfTimeScreen({
    required int Route_ID,
  }) async {
    ...
    await DioHelper.deleteData(
      ...
      print(value.data.toString());
      print('********************* DELETE ***************');
      emit(SuccessDeleteTimeScreenState());
    }).catchError((error) {
      print(error.toString());
      emit(ErrorDeleteTimeScreenState(error.toString()));
    });
  }
}
